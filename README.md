...
> A Quarto extension to generate a polished, two-column PDF résumé using
> Typst — fully configurable via YAML, no Typst knowledge required.

![Quarto ≥ 1.9](https://img.shields.io/badge/Quarto-%E2%89%A51.9-blue)
![Typst](https://img.shields.io/badge/Typst-PDF-orange.png) ![License:
CC BY 4.0](https://img.shields.io/badge/License-CC%20BY%204.0-green)

*Extension by [Valentin Goupille](https://www.valentingoupille.com) —
free to use, modify and distribute under the [CC BY 4.0
License](LICENSE).*
...
- **Two-column layout** — configurable sidebar (width, background,
...
PDFium engine directly in its Python wheel — **no system-level
dependency required**.
...
All contact fields are optional. Omit any field to hide it from the
sidebar.
...
Global spacing applied to all sidebar sections unless overridden per
section.
...
The `sidebar-sections` list controls which sections appear and in what
order. Each section has an `id`, an optional `title`, an `items` list,
and an optional `style` block.
...
These IDs have pre-registered style slots in the template (useful for
...
Wrap any section in `.timeline` to add dot/line indicators next to
headings:
...
Left column with text and an image.
...
Multi-column layouts also support tables, diagrams, and other Quarto
elements. For more details, see:
...
- [Custom Typst templates
  documentation](https://quarto.org/docs/output-formats/typst-custom.html)
- [Quarto 1.9 release
  announcement](https://quarto.org/docs/blog/posts/2026-03-24-1.9-release/)
...
Wraps `::: {.timeline}` divs into a `#timeline-section()` Typst call,
which draws a vertical line with dots aligned to heading levels.
...
By default all heading levels (H1–H6) inside the block receive a dot.
Use the `levels` attribute to target only certain levels:
...
The `timeline-section` function and its dot/line styling are defined in
`template.typ` and configurable via `cv-layout.timeline` in the YAML.
...
no system installation required. Any other font can be used by
installing it on the system and setting `main-font` in `cv-theme`.
...
request for bug fixes, new features, or improvements.
...
This project is released under the [CC BY 4.0 License](LICENSE) — you
are free to use, modify and distribute it, including for commercial
purposes, as long as you credit the original author.
